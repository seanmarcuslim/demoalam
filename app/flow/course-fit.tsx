...
import {
  courseFitFlow,
  getCourseFitLocalizedValue,
  getCourseFitOutcome,
  type CourseFitAnswerId,
...
export default function CourseFitFlowScreen() {
  ...
  const [answers, setAnswers] = useState<CourseFitAnswers>({})
  ...
  const result = useMemo(
    () => (completed ? getCourseFitOutcome(answers) : null),
    [answers, completed]
  )
  ...
  const handleAnswer = (answerId: CourseFitAnswerId) => {
  ...
  const renderIntro = () => (
    <AppCard style={styles.heroCard}>
      ...
      <SafeText variant="h2" weight="700" style={styles.cardTitle}>
        {getCourseFitLocalizedValue(courseFitFlow, 'title', language)}
      </SafeText>
      ...
      <AppCard style={styles.questionCard}>
        <View style={styles.progressRow}>
          <SafeText variant="caption" color="primary" weight="700">
            {language === 'fil' ? 'Check sa course' : 'Course check'}
          </SafeText>

          <SafeText variant="caption" color="muted">
            {`${questionIndex + 1}/${courseFitFlow.questions.length}`}
          </SafeText>
        </View>

        <SafeText variant="h3" weight="700" style={styles.questionText}>
          {getCourseFitLocalizedValue(currentQuestion, 'question', language)}
        </SafeText>
          ...
          {currentQuestion.options.map((option) => (
            ...
            >
              <SafeText variant="body" weight="700" style={styles.optionText}>
                {getCourseFitLocalizedValue(option, 'label', language)}
              </SafeText>
    ...
    return (
      <View>
        <AppCard style={styles.resultCard}>
          ...
          >
            {language === 'fil' ? 'Unang i-check' : 'First thing to check'}
          </SafeText>

          <SafeText variant="h2" weight="700" style={styles.resultTitle}>
            {getCourseFitLocalizedValue(outcome, 'first_action', language)}
          </SafeText>
          ...
          <SafeText variant="bodyMd" color="muted" style={styles.cardBody}>
            {getCourseFitLocalizedValue(outcome, 'why', language)}
          </SafeText>

          {outcome.mentor_note_en ? (
            <View style={styles.mentorBox}>
              ...
              <SafeText
                ...
                color="primary"
                weight="700"
                style={styles.mentorText}
              >
                {getCourseFitLocalizedValue(outcome, 'mentor_note', language)}
              </SafeText>
            </View>
          ) : null}
        ...
        <View style={styles.resultActions}>
          ...
          <AppButton
            title={language === 'fil' ? 'Bumalik sa Home' : 'Back to Home'}
            variant="secondary"
            onPress={() => router.push('/')}
            style={styles.secondaryAction}
          />
  ...
  return (
    <View style={styles.container}>
      <AppHeader
        title={language === 'fil' ? 'Check sa course fit' : 'Course fit check'}
        subtitle={
          language === 'fil'
            ? 'Alamin ang dapat i-check bago pumili.'
            : 'Know what to verify before choosing.'
        }
      />
