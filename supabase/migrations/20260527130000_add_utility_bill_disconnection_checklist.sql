...
values
  ...
  ((select id from public.guides where slug = 'utility-bill-disconnection-checklist'),'tip',9,
    '{"title":"Keep proof and follow up if payment does not show","body":"After paying, check your account status and keep the receipt, reference number, or confirmation. If payment does not appear within the expected posting time, contact provider support with your proof immediately.","items":["Keep the payment receipt or reference number","Check the account status after the expected posting window","Compare the amount paid against the bill details","Save the channel name and staff contact if available","Call the provider if the payment is missing or not posted"]}'::jsonb,
    '{"title":"Itabi ang proof at mag-follow up kung hindi lumabas ang bayad","body":"Pagkatapos magbayad, i-check ang account status at itabi ang resibo, reference number, o confirmation. Kung hindi lumabas ang bayad sa inaasahang posting time, kontakin agad ang provider support gamit ang proof.","items":["Itabi ang payment receipt o reference number","I-check ang account status pagkatapos ng inaasahang posting window","I-compare ang binayad sa detalye ng bill","I-save ang pangalan ng channel at contact ng staff kung meron","Tumawag sa provider kung nawawala o hindi na-post ang bayad"]}'::jsonb);
